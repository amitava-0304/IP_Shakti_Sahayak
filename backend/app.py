...
def process_uploaded_document(
    ...
    try:
        upload_jobs[job_id]["status"] = "indexing"
        upload_jobs[job_id]["message"] = "Document indexing is in progress."
        ...
        print(
            f"Upload indexing finished: {filename} -> {result}",
            flush=True
        )
        ...
        print(
            f"Upload indexing failed: {filename} -> {repr(error)}",
            flush=True
        )
...
async def upload_file(
    ...
    try:
        ...
        upload_jobs[job_id] = {
            ...
            "chunks": 0,
            "searchable": False,
            "message": "Document indexing is starting."
        }
...
def upload_status(
    ...
    end_time = job.get("finished_at") or time.time()
    ...
    return {
        ...
        "searchable": job.get("searchable", False),
